...
for i = 1:length(ExternalLoadNames)
    ...
    else V.ExternalLoads.objects.ExternalForce(i).point_expressed_in_body = PointExpressedinBody{i};
    end
    if isempty(ForceIdentifier)
        V.ExternalLoads.objects.ExternalForce(i).force_identifier = [ExternalLoadNames{i} '_ground_force_v'];
    else V.ExternalLoads.objects.ExternalForce(i).force_identifier = ForceIdentifier{i};
    end
    if isempty(PointIdentifier)
        V.ExternalLoads.objects.ExternalForce(i).point_identifier = [ExternalLoadNames{i} '_ground_force_p'];
    else V.ExternalLoads.objects.ExternalForce(i).point_identifier = PointIdentifier{i};
    end
    if isempty(TorqueIdentifier)
        V.ExternalLoads.objects.ExternalForce(i).torque_identifier = [ExternalLoadNames{i} '_ground_torque_'];
